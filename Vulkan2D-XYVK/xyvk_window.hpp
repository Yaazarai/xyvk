...
#define __XYVK_WINDOW
	...
	namespace XYVK_NAMESPACE {
		...
		public:
			...
			: hwndResizable(resizable), hwndBordered(bordered), hwndFullscreen(fullscreen), hwndMinSize(hasMinSize), hwndWidth(width), hwndHeight(height), minWidth(minWidth), minHeight(minHeight), hwndTitle(title), hwndWindow(VK_NULL_HANDLE), hwndSurface(VK_NULL_HANDLE) {
			...
			void OnFrameBufferReSizeCallback(int& width, int& height) {
				...
				hwndWidth = width, hwndHeight = height;
			}
			...
			VkSurfaceKHR CreateWindowSurface(VkInstance instance) {
				if (hwndSurface != VK_NULL_HANDLE) return hwndSurface;
				VkResult result = glfwCreateWindowSurface(instance, hwndWindow, VK_NULL_HANDLE, &hwndSurface);
				return (result == VK_SUCCESS)? hwndSurface : VK_NULL_HANDLE;
			}
